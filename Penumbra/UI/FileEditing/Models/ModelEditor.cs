...
public sealed partial class ModelEditor : IFileEditor
{
    ...
    public void Export(string outputPath, Utf8GamePath mdlPath)
    {
        ...
        try
        {
            sklbPaths = _models.ResolveSklbsForMdl(mdlPath.ToString(), GetCurrentEstManipulations());
        }
        ...
        BeginIo();
        _models.ExportToGltf(ExportConfig, Mdl, sklbPaths, ReadFile, outputPath)
    ...
    private void RecordIoExceptions(Exception? exception)
    {
        ...
        {
            case null:                  break;
            case AggregateException ae: IoExceptions.AddRange(ae.Flatten().InnerExceptions); break;
            default:                    IoExceptions.Add(exception); break;
        }
    }
