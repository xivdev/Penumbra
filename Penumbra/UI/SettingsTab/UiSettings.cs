...
public sealed class UiSettings(PersistentUiConfig config, IUiBuilder uiBuilder) : IUiService
{
    public void Draw()
    {
        using var header = Im.Tree.HeaderId("Interface"u8);
        if (!header)
            return;
    ...
    {
        using var tree = Im.Tree.Node("Filters"u8, TreeNodeFlags.DefaultOpen);
        if (!tree)
            return;
    ...
    private void DrawDisplaySettings()
    {
                ...
                config.HideChangedItemFilters))
        {
            ...
            if (v)
            {
                config.Filters.ModChangedItemTypeFilter = ChangedItemFlagExtensions.AllFlags;
                config.Filters.ChangedItemTypeFilter    = ChangedItemFlagExtensions.AllFlags;
        ...
        {
            config.HideMachinistOffhandFromChangedItems = v;
    ...
    private void DrawOptionGroupSettings()
    {
        ...
        Im.Item.SetNextWidth(UiHelpers.InputTextWidth.X);
        if (ImEx.InputOnDeactivation.Drag("##groupLine"u8, config.Ui.ModSettingLineScale,
                out var newLine, "%.2f"u8, 0, 4, 0.005f, SliderFlags.AlwaysClamp))
            config.Ui.ModSettingLineScale = newLine;
        ...
        Im.Item.SetNextWidth(UiHelpers.InputTextWidth.X);
        if (ImEx.InputOnDeactivation.Drag("##groupBorder"u8, config.Ui.ModSettingBorderScale,
                out var newBorder, "%.2f"u8, 1, 4, 0.005f, SliderFlags.AlwaysClamp))
            config.Ui.ModSettingBorderScale = newBorder;
        ...
        Im.Item.SetNextWidth(UiHelpers.InputTextWidth.X);
        if (ImEx.InputOnDeactivation.Drag("##vertSpace"u8, config.Ui.ModSettingItemSpacingFactor,
                out var newFactor, "%.2f"u8, 0, 10, 0.01f, SliderFlags.AlwaysClamp))
            config.Ui.ModSettingItemSpacingFactor = newFactor;
        ...
        Im.Item.SetNextWidth(UiHelpers.InputTextWidth.X);
        if (ImEx.InputOnDeactivation.Drag("##groupAlign"u8, config.Ui.ModSettingLabelAlignment,
                out var newAlignment, "%.2f"u8, 0, 1, 0.0005f, SliderFlags.AlwaysClamp))
            config.Ui.ModSettingLabelAlignment = newAlignment;
        ...
        Im.Item.SetNextWidth(UiHelpers.InputTextWidth.X);
        if (ImEx.InputOnDeactivation.Drag("##comboAlign"u8, config.Ui.ModSettingComboAlignment,
                out var newCombo, "%.2f"u8, 0, 1, 0.0005f, SliderFlags.AlwaysClamp))
            config.Ui.ModSettingComboAlignment = newCombo;
        ...
        Im.Item.SetNextWidth(UiHelpers.InputTextWidth.X);
        if (ImEx.InputOnDeactivation.Drag("##groupHomo"u8, config.Ui.ModSettingMaximumExtendLabelWidth,
                out var newExtend, "%.0f"u8, -1))
            config.Ui.ModSettingMaximumExtendLabelWidth = newExtend;
