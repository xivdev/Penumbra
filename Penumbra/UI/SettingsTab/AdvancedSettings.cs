...
public sealed class AdvancedSettings(AdvancedConfig config)
{
    ...
    private void DrawAdvancedSettings()
    {
        ...
        Checkbox("Auto Deduplicate on Import"u8,
            ...
            config.AutoDeduplicateOnImport, v => config.AutoDeduplicateOnImport = v);
        Checkbox("Auto Reduplicate UI Files on PMP Import"u8,
            ...
            config.AutoReduplicateUiOnImport, v => config.AutoReduplicateUiOnImport = v);
        DrawCompressionBox();
        Checkbox("Keep Default Metadata Changes on Import"u8,
          ...
          + "Toggle this to keep them, for example if an option in a mod is supposed to disable a metadata change from a prior option."u8,
            config.KeepDefaultMetaChanges, v => config.KeepDefaultMetaChanges = v);
        Checkbox("Enable Custom Shape and Attribute Support"u8,
            ...
            config.EnableCustomShapes, _attributeHook.SetState);
    ...
    private void DrawCompressionBox()
    {
        ...
        Checkbox("Use Filesystem Compression"u8,
            ...
            v =>
            {
                config.UseFileSystemCompression = v;
    ...
    {
        var warning = config.MinimumSize.X < Configuration.Constants.MinimumSizeX
            ? config.MinimumSize.Y < Configuration.Constants.MinimumSizeY
        ...
        if (ImEx.Button("Reset##resetMinSize"u8, new Vector2(buttonWidth / 2 - Im.Style.ItemSpacing.X * 2, 0),
                ...
                config.MinimumSize is { X: Configuration.Constants.MinimumSizeX, Y: Configuration.Constants.MinimumSizeY }))
        {
            config.MinimumSize = new Vector2(Configuration.Constants.MinimumSizeX, Configuration.Constants.MinimumSizeY);
            config.Save();
        }
...
#pragma warning restore CS0162 // Unreachable code detected
    ...
    {
        var tmp = config.DebugMode;
        if (Im.Checkbox("##debugMode"u8, ref tmp) && tmp != config.DebugMode)
        {
            config.DebugMode = tmp;
            config.Save();
        }
