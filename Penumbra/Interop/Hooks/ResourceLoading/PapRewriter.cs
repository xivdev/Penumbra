...
public sealed class PapRewriter(PeSigScanner sigScanner, PapRewriter.PapResourceHandlerPrototype papResourceHandler) : AsmHookBase(sigScanner)
{
    ...
    public void Rewrite(string sig, string name)
    {
        ...
        foreach (var hookPoint in hookPoints)
        {
            ...
            Setup($"{name}.PapRedirection", hookAddress, AssembleToBytes(assembler)).Enable();
    ...
    private void UpdatePathAddresses(IEnumerable<Instruction> stackAccesses, nint stringAllocation, string name)
    {
        foreach (var (index, stackAccess) in stackAccesses.Index())
        {
            ...
            if (Hooks.ContainsKey(hookAddress))
                continue;
            ...
            Setup($"{name}.PapStackAccess[{index}]", hookAddress, AssembleToBytes(assembler)).Enable();
        }
    }
